...
class BrokerServer(object):
    ...
    def __init__(self, host, port, in_options, in_world_session):
        ...
        print("Limite de TCP do Broker")
        #print("GunBound - Servidores: ")
        #for server_option in self.server_options:
            #print("Servidor:", server_option.server_name, "-", server_option.server_description, "na porta", server_option.server_port)

    def listen(self):
        self.sock.listen(5)
        print("BROKERSERVER: AGUARDANDO LOGS DA PORTA:", self.port)
        while True:
    ...
    def client_connection(self, client, address):
        print("BROKERSERVER: Nova conexao de", address)
        socket_rx_size = 1024
        ...
        while True:
            try:
                ...
                if data:
                    if len(data) < 6:
                        print("Pacote invalido, comprimento: " + len(data))
                        print(data)
                    else:
                        ...
                        if client_command == 0x1013:
                            print("BROKERSERVER: Solicitacao de autenticacao")
                            login_packet = BrokerServer.generate_packet(-1, 0x1312, int_to_bytes(0x0000, 2, big_endian=True))
                            client.send(login_packet)

                        elif client_command == 0x1100:
                            print("BROKERSERVER: Solicitacao de diretorio do servidor")
                    ...
                    print("BROKERSERVER: Cliente desconectado")
                    return True
            except:
                #client.close()
                print("ERRO: Cliente (BROKERSERVER) ignorando erro para continuar online.")
                #return True
